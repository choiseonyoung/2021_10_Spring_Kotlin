...
//    private lateinit var bDao : BuyerRepository
    ...
    override fun selectAll(pageable: Pageable): Page<Buyer> {
        ...
        // * bRepo에 매개변수 없는 것과 있는 것 이미 준비돼있음
    }
    ...
    // 새로운 고객ID(userid)를 생성하여 Buyer에 담아서 return
    override fun insert(): Buyer {
        var userid = bRepo.maxUserId()

        /**
         * Repo 에서 return 받은 userid 에서 1번 위치부터 (두 번째 문자열)
         * 잘라서 정수로 변환하여 userSeq 에 담아라
         * 만약 exception 이 발생하면 console 에 표시하고 0을 담아라
         */
        val userSeq = try {
            // B012
